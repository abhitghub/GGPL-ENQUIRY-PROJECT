...
_CHUNK_SIZE = max(1, int(os.environ.get('SMART_PARSE_CHUNK_SIZE', '20')))
_MAX_WORKERS = max(1, int(os.environ.get('SMART_PARSE_MAX_WORKERS', '4')))
_MIN_CALL_INTERVAL_SEC = max(0.0, float(os.environ.get('SMART_PARSE_MIN_CALL_INTERVAL_SEC', '0.25')))
...
def _get_redis():
    if os.environ.get('SMART_PARSE_REDIS_CACHE_ENABLED', '').strip().lower() not in ('1', 'true', 'yes'):
        return None
